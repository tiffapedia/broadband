...
```{r}
o <- data.frame(group="open access", value=open_countries$Penetration.per.100.OECD..2008)
no <- data.frame(group="non-open access", value=non_open_countries$Penetration.per.100.OECD..2008)
r1 <- rbind(o, no)
g1 <- ggplot(r1, aes(x=group, y=value)) + geom_boxplot() + labs(title="Penetration in 2008", x="", y="penetration per 100")
o <- data.frame(group="open access", value=open_countries$Household.penetration..OECD)
no <- data.frame(group="non-open access", value=non_open_countries$Household.penetration..OECD)
r2 <- rbind(o, no)
g2 <- ggplot(r2, aes(x=group, y=value)) + geom_boxplot() + labs(title="Household Penetration", x="", y="penetration per 100")
o <- data.frame(group="open access", value=open_countries$X3G.penetration.per.100)
no <- data.frame(group="non-open access", value=non_open_countries$X3G.penetration.per.100)
r3 <- rbind(o, no)
g3 <- ggplot(r3, aes(x=group, y=value)) + geom_boxplot() + labs(title="3G Penetration", x="", y="penetration per 100")
o <- data.frame(group="open access", value=open_countries$Wi.Fi.hotspots.per.100.000..JiWire)
no <- data.frame(group="non-open access", value=non_open_countries$Wi.Fi.hotspots.per.100.000..JiWire)
r4 <- rbind(o, no)
g4 <- ggplot(r4, aes(x=group, y=value)) + geom_boxplot() + labs(title="WiFi Hotspots Penetration", x="", y="penetration per 100")
grid.arrange(g1, g2, g3, g4, ncol=2, top="Comparing penetration for open access vs non-open access countries")
```
Non-open access countries appears to have a lower penetration in general as demonstrated from the visualization, but we cannot make this statement conclusively as we have only 27 datasets to work with for Open Access countries and 3 datasets to work with for Non-Open Access Countries. 
...
```{r}
o <- data.frame(group="open access", value=open_countries$Price.for.low.speeds..combined)
no <- data.frame(group="non-open access", value=non_open_countries$Price.for.low.speeds..combined)
r1 <- rbind(o, no)
g1 <- ggplot(r1, aes(x=group, y=value)) + geom_boxplot() + labs(title="Low Speed", x="", y="Price ($)")
o <- data.frame(group="open access", value=open_countries$Price.for.med.speeds..combined)
no <- data.frame(group="non-open access", value=non_open_countries$Price.for.med.speeds..combined)
r2 <- rbind(o, no)
g2 <- ggplot(r2, aes(x=group, y=value)) + geom_boxplot() + labs(title="Medium Speed", x="", y="Price($)")
o <- data.frame(group="open access", value=open_countries$Price.for.high.speeds..combined)
no <- data.frame(group="non-open access", value=non_open_countries$Price.for.high.speeds..combined)
r3 <- rbind(o, no)
g3 <- ggplot(r3, aes(x=group, y=value)) + geom_boxplot() + labs(title="High Speed", x="", y="Price($)")
o <- data.frame(group="open access", value=open_countries$Price.for.very.high.speeds..combined)
no <- data.frame(group="non-open access", value=non_open_countries$Price.for.very.high.speeds..combined)
r4 <- rbind(o, no)
g4 <- ggplot(r4, aes(x=group, y=value)) + geom_boxplot() + labs(title="Very High Speed", x="", y="penetration per 100")
grid.arrange(g1, g2, g3, g4, ncol=2, top="Comparing price for open access vs non-open access countries")
```

The graph demonstrates that Non-Open Access countries tend to price their network speed higher in particular with increasing speed. Again, we cannot conclusively state this due to the limited amount of data. 
...
We can begin to look at the advertised speed and compare that to the actual speed. As shown in the boxplot above, the advertised speed appears to be significantly higher than the actual speed. Most countries’ max advertised and average advertised speeds seem to be between 0-25000 kbps, whereas the actual speed 0-8000 kbps. Interesting to note is the claim advertisement seems to promise for its maximum advertised speed, going so far as having an outlier of 1,000,000 kbps. It appears to be outside the scope of what the actual speed can deliver. It comes as no surprise that the histogram for maximum advertised speed appears to be more positively skewed. In general, the maximum advertised speed and the average advertised speed have a wider spread due to their respective standard deviation being larger than the average actual speed; it can be assumed that advertisements are not limited by actual hardware limitations in terms of claimed speed. 
...
Network owners in US argue that adopting open access policies will create a trade-off between price, speed, and penetration. For this reason, we will look at the countries that do not adopt open access policies and see how the speed and penetration compare to countries that do have open access policies.
...
```
Overall, there appears to be a positive correlation between penetration per 100 inhabitants, house-hold penetration, 3G penetration per 100 inhabitants and wifi hotspots per 100,00 inhabitants and speed, with the correlation being highest between household penetration and average actual speed. In conclusion, countries with higher penetration also have relatively higher internet speeds and lower latencyin general. Since we have two to three datasets to work with for Non Open-Access countries, we cannot conclude whether or not open access policies enables higher penetration and faster speed. US appears to22be on the average in terms of penetration and speed while Mexico and Slovak Republic seem to be trailing behind in terms of both.
...
In conclusion, the higher the penetration the more affordable the price specifically for medium speed. Again, we cannot conclude with just three datasets whether or not Non-Open Access countries offer better price at a higher penetration.

## Speed and Price

As mentioned in univariate analysis, higher prices correlate with higher speed, and the two higher speed categories have a larger range of prices when compared to the low speed price data. 
...
The price variables within the dataset are associated with a speed category (low, medium, high and very high). The scatterplot above displays the actual speed vs medium-speed price variable. It appears that higher speeds tend to have lower price. Intuitively, this is the opposite of what is to be expected (higher speed correlates with higher price). However the scatterplot only uses one price category (medium) and compares over the actual speed dataset. The dataset provided did not clearly state how many kbps were considered in each of the price categories. This makes copmaring speed vs price more complicated as the specific speed within the "medium speed price" level per country is not given. 
...
We have learned that most of the countries surveyed in this dataset have a relatively increased penetration from the year 2007 to 2008. Mobile connection has a relatively higher penetration and growth rate than other network connection. 

Higher price seems to correlate with higher speed. More countries tend to not implement the high or very high infrastructure. If the higher speed is not implemented, the lower speed tends to be priced at a higher than average price. Speed tends to be advertised higher than the actual speed, which is to be expected. 

It appears that penetration is lower in Non-Open Access countries priced at a relatively higher price and lower speed. But we cannot make a conclusive statement based on this data that open access policy provides customers with better service with higher penetration and speed at a lower price because there are only three Non-Open Access countries. 

In conclusion, countries with higher penetration seem to have relatively higher internet speeds in all benchmarks (downloads, uploads, actual speed tests) and lower latency. Additionally, there appears to be a negative relationship between overall penetration and price for medium speed. In regards to speed and price, however, the relationship is unclear. When comparing medium speed price and speed, there is a negative correlation that may be explained by infrastructural differences that may allow more established broadband networks to offer higher speeds at lower prices. However, when looking at speed and price at the respective levels, price seems to increase with speed. Further information on the explicit speeds per low, medium, high and very high categories could help clarify this contradiction.

When looking at the effects of open access versus non open-access policies, we must view the conclusions with caution as the outcomes are based on data from 2-3 countries, namely Mexico, Slovak Rep. and the U.S. With this in mind, we did find that in 82% of the categoriesmeasured in the dataset, the Open Access countries had a superior mean. Superior is interpreted to be high speed, larger penetration and smaller price variables. Thus, from the analysis performed, Open Access Countries appear to provide superior Broadband metrics to their citizens.
