...
Difference
```
Interestingly enough, there is one country that experienced negative growth.
```{r}
...
We will look into the most recent penetration data Penetration.per.100.OECD..2008, which reflects both businesses and residences before diving in to penetration data in household (Household.penetration..OECD), in mobile 3G (X3G.penetration.per.100), and in WiFi hotspots (Wi.Fi.hotspots.per.100.000..JiWire). We will compare it to the average actual speed (Average.actual.speed..Akamai..kbps.). Then, we will look at the relationship of penetration and  price for medium speed as well as speed and price for medium speed since the data for price for medium speed is most complete. (Price.for.med.speeds..combined).
